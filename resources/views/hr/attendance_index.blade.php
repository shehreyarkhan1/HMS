...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <p class="mb-0" style="font-size:13px;color:#94a3b8">{{ \Carbon\Carbon::create($year, $month)->format('F Y') }}
                attendance records</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('hr.attendance.monthly', ['month' => $month, 'year' => $year]) }}"
            ...
            <div class="stat-card">
                <div class="d-flex align-items-start justify-content-between">
                    <div>
                        ...
                        <div class="stat-sub">incl. late & WFH</div>
                    </div>
        ...
        <div class="col-6 col-xl-3">
            <div class="stat-card">
                <div class="d-flex align-items-start justify-content-between">
                    ...
                    <div class="stat-icon" style="background:#fef3c7;color:#b45309"><i class="bi bi-clock-history"></i>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="att-row">
                ...
                <div class="att-info">
                    ...
                    <div class="att-meta">
                        ...
                        @if ($att->check_in)
                            &middot; In: <strong>{{ \Carbon\Carbon::parse($att->check_in)->format('H:i') }}</strong>
                        @endif
                        @if ($att->check_out)
                            &middot; Out: <strong>{{ \Carbon\Carbon::parse($att->check_out)->format('H:i') }}</strong>
                        @endif
                        @if ($att->working_minutes)
                            &middot; {{ floor($att->working_minutes / 60) }}h {{ $att->working_minutes % 60 }}m
                        @endif
                    </div>
                </div>
                @if ($att->overtime_minutes > 0)
                    <span class="time-pill"><i class="bi bi-plus-circle me-1"></i>OT
                        {{ floor($att->overtime_minutes / 60) }}h {{ $att->overtime_minutes % 60 }}m</span>
                @endif
                @if ($att->late_minutes > 0)
                    <span class="time-pill" style="background:#fef3c7;color:#b45309"><i class="bi bi-clock me-1"></i>Late
                        {{ $att->late_minutes }}m</span>
                @endif
                <span class="status-badge {{ $statusClass }}">{{ $att->status }}</span>
                <span class="source-icon" title="{{ $att->source }}">
                    <i
                        class="bi bi-{{ $att->source === 'Biometric' ? 'fingerprint' : ($att->source === 'System' ? 'cpu' : 'pencil') }}"></i>
                </span>
                @if ($att->is_regularized)
                    <span style="font-size:10px;color:#7c3aed;font-weight:600"
                        title="{{ $att->regularization_reason }}"><i class="bi bi-pencil-square"></i> R</span>
                @endif
                <button class="btn btn-sm btn-outline-secondary py-0 px-2" style="font-size:12px" data-bs-toggle="modal"
                    data-bs-target="#regularizeModal" data-id="{{ $att->id }}" data-status="{{ $att->status }}"
                    data-check_in="{{ $att->check_in }}" data-check_out="{{ $att->check_out }}">
            ...
            <div class="modal-content" style="border-radius:14px;border:1px solid #e2e8f0">
                <div class="modal-header" style="border-bottom:1px solid #f1f5f9;padding:1.1rem 1.5rem">
                    <h6 class="modal-title fw-semibold" style="color:#1e293b"><i class="bi bi-plus-circle me-2"
                            style="color:#1d4ed8"></i>Mark Attendance</h6>
                ...
                <form action="{{ route('hr.attendance.store') }}" method="POST">
                    ...
                    <div class="modal-body" style="padding:1.5rem">
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label" style="font-size:12px;font-weight:600;color:#64748b">Employee
                            ...
                            <div class="col-6">
                                <label class="form-label" style="font-size:12px;font-weight:600;color:#64748b">Date <span
                                        class="text-danger">*</span></label>
                                <input type="date" name="date" class="form-control"
                                    value="{{ today()->toDateString() }}" max="{{ today()->toDateString() }}" required>
                            </div>
                            <div class="col-6">
                                <label class="form-label" style="font-size:12px;font-weight:600;color:#64748b">Status
                                    <span class="text-danger">*</span></label>
                            ...
                            <div class="col-6">
                                <label class="form-label" style="font-size:12px;font-weight:600;color:#64748b">Check
                                    In</label>
                            ...
                            <div class="col-12">
                                <label class="form-label"
                                    style="font-size:12px;font-weight:600;color:#64748b">Notes</label>
            ...
            <div class="modal-content" style="border-radius:14px;border:1px solid #e2e8f0">
                <div class="modal-header" style="border-bottom:1px solid #f1f5f9;padding:1.1rem 1.5rem">
                    <h6 class="modal-title fw-semibold" style="color:#1e293b"><i class="bi bi-pencil-square me-2"
                            style="color:#7c3aed"></i>Regularize Attendance</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="regularizeForm" method="POST">
                    ...
                    <div class="modal-body" style="padding:1.5rem">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label class="form-label" style="font-size:12px;font-weight:600;color:#64748b">Reason
                                    <span class="text-danger">*</span></label>
                                <textarea name="regularization_reason" class="form-control" rows="2" placeholder="Reason for regularization…"
                                    required></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer" style="border-top:1px solid #f1f5f9;padding:1rem 1.5rem">
                        ...
                        <button type="submit" class="btn btn-warning btn-sm text-white"><i
                                class="bi bi-check-lg me-1"></i>Regularize</button>
                    </div>
                </form>
            </div>
        </div>
