...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            {{-- Top row mein yeh button add karo --}}
            <form action="{{ route('hr.leaves.balances.allocate') }}" method="POST" class="d-inline">
                @csrf
                <input type="hidden" name="year" value="{{ $year }}">
                <button type="submit" class="btn btn-success btn-sm"
                    onclick="return confirm('Allocate balances for {{ $year }}?')">
                    <i class="bi bi-plus-circle me-1"></i> Allocate {{ $year }} Balances
                </button>
            </form>
        ...
        <div class="d-flex gap-2 align-items-center">
            ...
            <a href="{{ route('hr.leaves.index') }}" class="btn btn-light btn-sm"><i
    ...
    <div class="section-card">
        <div class="section-header">
            <span class="section-title"><i class="bi bi-bar-chart me-2" style="color:#1d4ed8"></i>{{ $year }} Leave
                Balances</span>
                ...
                <thead>
                    <tr>
                        ...
                        @foreach ($leaveTypes as $lt)
                            <th class="text-center">{{ $lt->code }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="emp-cell">
                                    ...
                                    <div>
                                        ...
                                        <div style="font-size:11px;color:#94a3b8">{{ $emp->employee_id }} &middot;
                                            {{ $emp->department }}</div>
                                    </div>
                                </div>
                            </td>
                            @foreach ($leaveTypes as $lt)
                                ...
                                <td class="text-center">
                                    @if ($bal)
                                        <span
                                            class="bal-badge {{ $cls }}">{{ $rem }}/{{ $alloc }}</span>
                                        <div class="progress-thin">
                                            <div class="progress-fill"
                                                style="width:{{ $pct }}%;background:{{ $rem <= 0 ? '#dc2626' : ($pct >= 70 ? '#b45309' : '#1d4ed8') }}">
                                            </div>
                                        </div>
                                    @else
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $leaveTypes->count() + 1 }}" class="text-center py-4" style="color:#94a3b8">No
                                employees found.</td>
                        </tr>
                    @endforelse
                </tbody>
