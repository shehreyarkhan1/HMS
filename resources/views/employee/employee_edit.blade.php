    ...
    <form method="POST" action="{{ route('employees.update', $employee->id) }}" enctype="multipart/form-data">
        ...
        <div class="row g-3">
            <div class="col-12 col-lg-8">
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <div class="section-icon" style="background:#eff6ff"><i class="bi bi-person"
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="marital_status" class="form-select">
                                    @foreach (['Single', 'Married', 'Divorced', 'Widowed'] as $m)
                                        <option value="{{ $m }}"
                                            {{ old('marital_status', $employee->marital_status) == $m ? 'selected' : '' }}>
                                            {{ $m }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <div class="section-icon" style="background:#fff7ed"><i class="bi bi-briefcase"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="designation" class="form-select @error('designation') is-invalid @enderror">
                                    @foreach ($designations as $des)
                                        <option value="{{ $des }}"
                                            {{ old('designation', $employee->designation) == $des ? 'selected' : '' }}>
                                            {{ $des }}</option>
                                    @endforeach
                                </select>
                                @error('designation')
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="employment_type" class="form-select">
                                    @foreach (['Permanent', 'Contractual', 'Probationary', 'Part-Time', 'Intern', 'Daily-Wage'] as $t)
                                        <option value="{{ $t }}"
                                            {{ old('employment_type', $employee->employment_type) == $t ? 'selected' : '' }}>
                                            {{ $t }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="shift" class="form-select">
                                    @foreach (['Morning', 'Evening', 'Night', 'Rotating', 'Custom'] as $s)
                                        <option value="{{ $s }}"
                                            {{ old('shift', $employee->shift) == $s ? 'selected' : '' }}>
                                            {{ $s }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="salary_type" class="form-select">
                                    @foreach (['Monthly', 'Daily', 'Hourly'] as $st)
                                        <option value="{{ $st }}"
                                            {{ old('salary_type', $employee->salary_type) == $st ? 'selected' : '' }}>
                                            {{ $st }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="number" name="basic_salary" class="form-control"
                                    value="{{ old('basic_salary', $employee->basic_salary) }}" min="0"
                                    step="500">
                            </div>
                            <div class="col-12">
                                ...
                                <div class="d-flex gap-2 flex-wrap mt-1">
                                    @foreach (['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $day)
                                        <div>
                                            ...
                                            <span
                                                class="day-btn {{ in_array($day, old('working_days', $employee->working_days ?? [])) ? 'selected' : '' }}"
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="form-card">
                    <div class="form-card-header">
                        <div class="section-icon" style="background:#fffbeb"><i class="bi bi-camera"
                                style="color:#d97706"></i></div>
                        <h6>Photo</h6>
                    </div>
                    <div class="form-card-body text-center">
                        ...
                        @else
                            <img id="photoPreview"
                                style="width:70px;height:70px;border-radius:50%;display:none;margin:0 auto 10px">
                        @endif
                ...
                <div class="form-card" style="border-color:#bfdbfe">
                    <div class="form-card-header" style="background:#eff6ff">
                        <div class="section-icon" style="background:#dbeafe"><i class="bi bi-shield-lock"
                                style="color:#1d4ed8"></i></div>
                    ...
                    <div class="form-card-body">
                        <div class="form-check">
                            ...
                            <label class="form-check-label" for="has_system_access_edit"
