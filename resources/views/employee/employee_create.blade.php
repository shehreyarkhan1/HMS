            ...
            <div class="col-12 col-lg-8">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="marital_status" class="form-select">
                                    @foreach (['Single', 'Married', 'Divorced', 'Widowed'] as $m)
                                        <option value="{{ $m }}"
                                            {{ old('marital_status', 'Single') == $m ? 'selected' : '' }}>
                                            {{ $m }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="blood_group" class="form-select">
                                    ...
                                    @foreach (['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'] as $bg)
                                        <option value="{{ $bg }}"
                                            {{ old('blood_group') == $bg ? 'selected' : '' }}>{{ $bg }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <input type="text" name="cnic"
                                    ...
                                    placeholder="3520112345678" maxlength="13">
                                @error('cnic')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="form-card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea name="present_address" rows="2" class="form-control" placeholder="Current address">{{ old('present_address') }}</textarea>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Permanent address</label>
                                <textarea name="permanent_address" rows="2" class="form-control" placeholder="Permanent address">{{ old('permanent_address') }}</textarea>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="department" class="form-select @error('department') is-invalid @enderror">
                                    ...
                                    @foreach ($departments as $dept)
                                        <option value="{{ $dept }}"
                                            {{ old('department') == $dept ? 'selected' : '' }}>{{ $dept }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('department')
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="designation" class="form-select @error('designation') is-invalid @enderror">
                                    ...
                                    @foreach ($designations as $des)
                                        <option value="{{ $des }}"
                                            {{ old('designation') == $des ? 'selected' : '' }}>{{ $des }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('designation')
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="employment_type"
                                    ...
                                    @foreach (['Permanent', 'Contractual', 'Probationary', 'Part-Time', 'Intern', 'Daily-Wage'] as $t)
                                        <option value="{{ $t }}"
                                            {{ old('employment_type', 'Permanent') == $t ? 'selected' : '' }}>
                                            {{ $t }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="employment_status" class="form-select">
                                    @foreach (['Active', 'On Leave', 'Suspended', 'Terminated', 'Resigned', 'Retired'] as $s)
                                        <option value="{{ $s }}"
                                            {{ old('employment_status', 'Active') == $s ? 'selected' : '' }}>
                                            {{ $s }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="shift" class="form-select">
                                    @foreach (['Morning', 'Evening', 'Night', 'Rotating', 'Custom'] as $s)
                                        <option value="{{ $s }}"
                                            {{ old('shift', 'Morning') == $s ? 'selected' : '' }}>{{ $s }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <div class="d-flex gap-2 flex-wrap mt-1">
                                    @foreach (['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'] as $day)
                                        <div>
                                            ...
                                            <span
                                                class="day-btn {{ in_array($day, old('working_days', ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'])) ? 'selected' : '' }}"
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="graduation_year" class="form-control"
                                        value="{{ old('graduation_year') }}" min="1970" max="{{ date('Y') }}"
                                        placeholder="{{ date('Y') }}">
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select name="salary_type" class="form-select">
                                        @foreach (['Monthly', 'Daily', 'Hourly'] as $st)
                                            <option value="{{ $st }}"
                                                {{ old('salary_type', 'Monthly') == $st ? 'selected' : '' }}>
                                                {{ $st }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="col-12 col-lg-4">
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-card-body">
                            <img id="photoPreview"
                                style="width:80px;height:80px;border-radius:50%;object-fit:cover;display:none;margin:0 auto 12px;border:3px solid #e2e8f0">
                            <div class="photo-upload" onclick="document.getElementById('photoInput').click()">
                                <i class="bi bi-camera"
                                    style="font-size:28px;color:#94a3b8;display:block;margin-bottom:6px"></i>
                    ...
                    <div class="form-card" style="border-color:#bfdbfe">
                        ...
                        <div class="form-card-body">
                            <div style="font-size:13px;color:#374151;margin-bottom:10px;line-height:1.6">
                                If this employee needs to login to HMS, enable system access. A user account can be created
                                separately from Users module.
                            </div>
                            <div class="form-check">
                                <input type="checkbox" name="has_system_access" id="has_system_access"
                                    class="form-check-input" value="1"
                                    {{ old('has_system_access') ? 'checked' : '' }}>
                                <label class="form-check-label" for="has_system_access"
