        ...
        <form method="POST" action="{{ route('appointments.store') }}" novalidate>
            ...
            <div class="form-card">
                <div class="form-card-header">
                    <div class="section-icon" style="background:#dbeafe;color:#1d4ed8"><i class="bi bi-person-fill"></i>
                    </div>
...
@push('scripts')
    <script>
        ...
        function checkAvailability() {
            ...
            indicator.innerHTML =
                '<span class="avail-badge avail-loading"><i class="bi bi-hourglass-split"></i> Checking…</span>';
            ...
            fetch(`{{ route('appointments.availability') }}?doctor_id=${doctorId}&date=${date}`)
                ...
                .then(data => {
                    ...
                    if (conflict) {
                        indicator.innerHTML =
                            '<span class="avail-badge avail-conflict"><i class="bi bi-x-circle-fill"></i> Time slot already booked</span>';
                    } else {
                        indicator.innerHTML =
                            `<span class="avail-badge avail-free"><i class="bi bi-check-circle-fill"></i> Slot available · ${data.total_booked} appointments that day</span>`;
                    }
                })
                .catch(() => {
                    indicator.style.display = 'none';
