        ...
        <div class="card-section-header">
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <form method="GET" action="{{ route('appointments.index') }}" class="filter-bar">
                    <input type="text" name="search" placeholder="Patient name / MRN..."
                           value="{{ request('search') }}" style="width:200px">

                    <input type="date" name="date" value="{{ request('date') }}" style="width:145px">

                    <select name="doctor_id" style="width:160px">
                ...
                <a href="{{ route('appointments.calendar') }}"
                   class="btn btn-sm btn-outline-primary px-3" style="height:36px;font-size:13px">
                ...
                <tbody>
                    ...
                    <tr class="{{ $appt->is_today ? 'row-today' : '' }}">
                        ...
                        <td>
                            {{-- Quick status update --}}
                            <form method="POST"
                                  action="{{ route('appointments.status', $appt->id) }}">
                                @csrf @method('PATCH')
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-5" style="color:#94a3b8">
                            ...
                            <a href="{{ route('appointments.create') }}" class="text-primary ms-1">Book first appointment</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
