...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <a href="{{ route('ot.index') }}" class="btn btn-sm btn-outline-secondary px-3" style="height:34px;font-size:13px">
            <i class="bi bi-arrow-left me-1"></i>Back to OT
        </a>
        <button type="button" class="btn btn-sm btn-success px-3" style="height:34px;font-size:13px" data-bs-toggle="modal"
    ...
    {{-- @if (session('success'))
        <div class="alert alert-success alert-dismissible fade show" style="font-size:13px;border-radius:10px"
            role="alert">
                    ...
                    <div class="room-card">
                        ...
                        <div class="d-flex align-items-start justify-content-between mb-3">
                            <div>
                                ...
                                <div style="font-size:14px;font-weight:600;color:#1e293b;margin-top:6px">{{ $room->name }}
                                </div>
                        ...
                        <div style="background:#f8fafc;border-radius:8px;padding:10px;margin-bottom:12px">
                            <div
                                style="font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">
                                Equipment</div>
                            <div class="d-flex flex-wrap gap-x-3 gap-y-1" style="gap:6px">
                                <span class="feature-label">
                                    <span
                                        class="feature-dot {{ $room->has_anesthesia_machine ? 'feature-on' : 'feature-off' }}"></span>
                                    Anesthesia
                                </span>
                                <span class="feature-label">
                                    <span
                                        class="feature-dot {{ $room->has_ventilator ? 'feature-on' : 'feature-off' }}"></span>
                                    Ventilator
                                </span>
                                <span class="feature-label">
                                    <span
                                        class="feature-dot {{ $room->has_laparoscopy ? 'feature-on' : 'feature-off' }}"></span>
                                    Laparoscopy
                                </span>
                                <span class="feature-label">
                                ...
                                <span class="feature-label">
                                    <span
                                        class="feature-dot {{ $room->is_laminar_flow ? 'feature-on' : 'feature-off' }}"></span>
                                    Laminar Flow
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-outline-warning py-0 px-2 edit-room-btn"
                                style="font-size:12px" data-bs-toggle="modal" data-bs-target="#editRoomModal"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius:16px;border:none">
                ...
                <form method="POST" action="{{ route('ot.rooms.store') }}">
                    @csrf
                    <input type="hidden" name="_modal" value="add"> {{-- add karo --}}
                    <div class="modal-body" style="padding:24px">
                        @include('operationtheater.otroom_form', ['editing' => false, 'room' => null])
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="border-radius:16px;border:none">
                ...
                <form method="POST" id="editRoomForm" action="">
                    @csrf @method('PUT')
                    <input type="hidden" name="_modal" value="edit"> {{-- add karo --}}
                    <input type="hidden" name="_edit_room_id" value=""> {{-- room id store karega --}}
                    <div class="modal-body" style="padding:24px">
                        @include('operationtheater.otroom_form', ['editing' => true, 'room' => null])
                    </div>
                    <div class="modal-footer" style="border-top:1px solid #f1f5f9;padding:16px 24px">
                        <button type="button" class="btn btn-outline-secondary btn-sm"
                            data-bs-dismiss="modal">Cancel</button>
    ...
    <script>
        document.querySelectorAll('.edit-room-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // Checkboxes
                ['has_anesthesia_machine', 'has_ventilator', 'has_laparoscopy', 'has_c_arm',
                    'is_laminar_flow', 'is_active'
                ].forEach(f => {
