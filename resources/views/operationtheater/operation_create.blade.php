                            ...
                            <select name="ot_room_id" class="form-select">
                                ...
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->id }}"
                                        {{ old('ot_room_id') == $room->id ? 'selected' : '' }}>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="diagnosis" class="form-control" value="{{ old('diagnosis') }}"
                                placeholder="Pre-operative diagnosis" required>
                            @error('diagnosis')
                                <div class="text-danger" style="font-size:11px;margin-top:4px">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-12">
                            ...
                            <input type="text" name="procedure_name" class="form-control"
                                ...
                                placeholder="e.g. Appendectomy, CABG, Laparoscopic Cholecystectomy" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="surgery_type" class="form-select" required>
                                @foreach (['Elective', 'Urgent', 'Emergency', 'Diagnostic'] as $t)
                                    <option value="{{ $t }}"
                                        {{ old('surgery_type', 'Elective') == $t ? 'selected' : '' }}>{{ $t }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="priority" class="form-select" required>
                                @foreach (['Routine', 'Priority', 'Urgent', 'Emergency'] as $p)
                                    <option value="{{ $p }}"
                                        {{ old('priority', 'Routine') == $p ? 'selected' : '' }}>{{ $p }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="anesthesia_type" class="form-select">
                                ...
                                @foreach (['General', 'Local', 'Regional', 'Spinal', 'Epidural', 'Sedation', 'None'] as $a)
                                    <option value="{{ $a }}"
                                        {{ old('anesthesia_type') == $a ? 'selected' : '' }}>{{ $a }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="form-section">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="notes" class="form-control" rows="2" placeholder="Any special requirements or notes...">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
                {{-- ── OT CHARGES ─────────────────────────────────────────────────── --}}
                        ...
                        <div class="team-row">
                            <div class="row g-2 align-items-end">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="team[__INDEX__][role]" required class="form-select form-select-sm">
                                ...
                                <!-- Doctor Dropdown (For Assistants/Observers) -->
                                <div class="col-md-4">
                                ...
                                <!-- Staff Dropdown (For Nurses/Technicians) -->
                                <div class="col-md-4">
            ...
            <div class="col-12 col-xl-4">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="surgeon_id" class="form-select" required>
                            ...
                            @foreach ($surgeons as $doc)
                                <option value="{{ $doc->id }}"
                                    {{ old('surgeon_id') == $doc->id ? 'selected' : '' }}>
                                    Dr. {{ $doc->employee?->first_name }} {{ $doc->employee?->last_name }}
                                    — {{ $doc->specialization }}
                                </option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <select name="anesthesiologist_id"
                            ...
                            @foreach ($anesthesiologists as $doc)
                                <option value="{{ $doc->id }}"
                                    {{ old('anesthesiologist_id') == $doc->id ? 'selected' : '' }}>
                                    Dr. {{ $doc->employee?->first_name }} {{ $doc->employee?->last_name }}
                                    ({{ $doc->specialization }})
                                    {{-- Optional: Sath specialization dikhane ke liye --}}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="consent_by" class="form-control" value="{{ old('consent_by') }}"
                            placeholder="Patient name or guardian name">
                    </div>
                    <div>
                ...
                <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px">
                    ...
                    <a href="{{ route('ot.index') }}" class="btn btn-outline-secondary w-100"
