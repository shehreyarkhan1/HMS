...
@push('styles')
    <style>
        ...
        .surgery-card.priority-Emergency {
            border-left: 3px solid #dc2626;
        }

        .surgery-card.priority-Urgent {
            border-left: 3px solid #f59e0b;
        }

        .surgery-card.priority-Priority {
...
@section('content')
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:14px 20px;margin-bottom:16px">
        <form method="GET" action="{{ route('ot.index') }}" class="d-flex align-items-center gap-2 flex-wrap filter-bar">

            <input type="text" name="search" placeholder="Search surgery ID, procedure, patient..."
                value="{{ request('search') }}" style="width:230px">

            <input type="date" name="date" value="{{ request('date') }}" title="Filter by date">
            ...
            <select name="surgery_type">
                ...
                @foreach (['Elective', 'Urgent', 'Emergency', 'Diagnostic'] as $t)
                    <option value="{{ $t }}" {{ request('surgery_type') == $t ? 'selected' : '' }}>
                        {{ $t }}</option>
                @endforeach
            </select>
                    ...
                    <div class="surgery-card {{ $priorityBorder }}">
                        ...
                        <div class="d-flex gap-1 mb-2 flex-wrap">
                            ...
                            @if ($schedule->anesthesia_type)
                                <span
                                    style="background:#f8fafc;color:#64748b;font-size:10px;padding:2px 8px;border-radius:6px;font-weight:500">
                        ...
                        <div class="mb-2">
                            ...
                            <div class="info-row">
                                ...
                                Dr. {{ $schedule->surgeon->employee?->first_name }}
                                {{ $schedule->surgeon->employee?->last_name }}
                            </div>
                            @if ($schedule->anesthesiologist)
                                <div class="info-row">
                                    <i class="bi bi-lungs"></i>
                                    Dr. {{ $schedule->anesthesiologist->employee?->first_name }}
                                    {{ $schedule->anesthesiologist->employee?->last_name }}
                        ...
                        {{-- Diagnosis --}}
                        <div
                            style="background:#f8fafc;border-radius:8px;padding:8px 10px;font-size:12px;color:#475569;margin-bottom:10px">
                            <span style="color:#94a3b8">Dx:</span> {{ Str::limit($schedule->diagnosis, 60) }}
                        </div>
                        {{-- Charges summary --}}
                        @if ($schedule->surgeon_fee > 0 || $schedule->ot_room_charges > 0)
                            <div class="d-flex justify-content-between align-items-center mb-2"
                                style="background:#f8fafc;border-radius:6px;padding:6px 10px">
                                <span style="font-size:11px;color:#94a3b8">Est. OT Cost</span>
                                <span style="font-size:12px;font-weight:600;color:#6366f1">
                                    Rs. {{ number_format($schedule->total_ot_cost, 2) }}
                                </span>
                            </div>
                        @endif
                        ...
                        <div class="d-flex gap-2 mb-3">
                            <span
                                style="font-size:10px;padding:2px 8px;border-radius:6px;font-weight:600;
                        ...
                        {{ $schedule->pre_op_assessment_done ? 'background:#dcfce7;color:#166534' : 'background:#fef9c3;color:#854d0e' }}">
                                <i
                                    class="bi bi-{{ $schedule->pre_op_assessment_done ? 'check' : 'clock' }}-circle me-1"></i>
                                Pre-op
                            </span>
                        </div>
                            ...
                            @if (in_array($schedule->status, ['Scheduled', 'Confirmed', 'Preparing', 'In-Progress']))
                                <div class="dropdown">
                                    <button class="btn btn-sm btn-outline-secondary py-0 px-2 dropdown-toggle"
                                        style="font-size:12px" data-bs-toggle="dropdown">
                                        Update Status
                                    </button>
                                            ...
                                            @if ($st !== $schedule->status)
                                                <li>
                                                    <button class="dropdown-item quick-status-btn"
                                                        data-id="{{ $schedule->id }}" data-status="{{ $st }}">
                                                        {{ $st }}
                                                    </button>
                                                </li>
                                            @endif
