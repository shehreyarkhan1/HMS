            ...
            <div class="col-12 col-xl-8">
                            ...
                            <select name="ot_room_id" class="form-select">
                                ...
                                @foreach ($rooms as $room)
                                    <option value="{{ $room->id }}"
                                        {{ $ot->ot_room_id == $room->id ? 'selected' : '' }}>
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="diagnosis" class="form-control"
                                value="{{ old('diagnosis', $ot->diagnosis) }}" required>
                        </div>
                        <div class="col-12">
                            ...
                            <input type="text" name="procedure_name" class="form-control"
                                value="{{ old('procedure_name', $ot->procedure_name) }}" required>
                        </div>
                        <div class="col-12">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="surgery_type" class="form-select" required>
                                @foreach (['Elective', 'Urgent', 'Emergency', 'Diagnostic'] as $t)
                                    <option value="{{ $t }}"
                                        {{ old('surgery_type', $ot->surgery_type) == $t ? 'selected' : '' }}>
                                        {{ $t }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="priority" class="form-select" required>
                                @foreach (['Routine', 'Priority', 'Urgent', 'Emergency'] as $p)
                                    <option value="{{ $p }}"
                                        {{ old('priority', $ot->priority) == $p ? 'selected' : '' }}>{{ $p }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="anesthesia_type" class="form-select">
                                ...
                                @foreach (['General', 'Local', 'Regional', 'Spinal', 'Epidural', 'Sedation', 'None'] as $a)
                                    <option value="{{ $a }}"
                                        {{ old('anesthesia_type', $ot->anesthesia_type) == $a ? 'selected' : '' }}>
                                        {{ $a }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="scheduled_date" class="form-control"
                                value="{{ old('scheduled_date', $ot->scheduled_date->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="time" name="scheduled_time" class="form-control"
                                value="{{ old('scheduled_time', substr($ot->scheduled_time, 0, 5)) }}" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" name="estimated_duration_mins" class="form-control"
                                value="{{ old('estimated_duration_mins', $ot->estimated_duration_mins) }}" min="10"
                                max="600" required>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="post_op_destination" class="form-select">
                                ...
                                @foreach (['Recovery Room', 'ICU', 'HDU', 'General Ward', 'Discharged'] as $dest)
                                    <option value="{{ $dest }}"
                                        {{ old('post_op_destination', $ot->post_op_destination) == $dest ? 'selected' : '' }}>
                                        {{ $dest }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="form-section">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="surgeon_fee" class="form-control"
                                    value="{{ old('surgeon_fee', $ot->surgeon_fee) }}" min="0" step="0.01"
                                    placeholder="0.00">
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="anesthesia_fee" class="form-control"
                                    value="{{ old('anesthesia_fee', $ot->anesthesia_fee) }}" min="0"
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="ot_room_charges" class="form-control"
                                    value="{{ old('ot_room_charges', $ot->ot_room_charges) }}" min="0"
                                    step="0.01" placeholder="0.00">
                            </div>
                        </div>
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="consumables_charges" class="form-control"
                                    value="{{ old('consumables_charges', $ot->consumables_charges) }}" min="0"
                                    step="0.01" placeholder="0.00">
                            </div>
                        </div>
                    </div>
                    <div class="mt-3 p-2 rounded d-flex justify-content-between align-items-center"
                        ...
                        <span style="font-size:12px;color:#94a3b8">Total OT Cost</span>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="team[{{ $i }}][employee_id]"
                                            ...
                                            @foreach ($employees as $emp)
                                                <option value="{{ $emp->id }}"
                                                    {{ $member->employee_id == $emp->id ? 'selected' : '' }}>
            ...
            <div class="col-12 col-xl-4">
                ...
                <div class="form-section">
                    ...
                    <select name="status" class="form-select" id="status-select" required>
                        @foreach (['Scheduled', 'Confirmed', 'Preparing', 'In-Progress', 'Completed', 'Postponed', 'Cancelled'] as $s)
                            <option value="{{ $s }}" {{ old('status', $ot->status) == $s ? 'selected' : '' }}>
                                {{ $s }}</option>
                        @endforeach
                    </select>
                    ...
                    <div id="reason-box" class="mt-3" style="display:none">
                        ...
                        <input type="text" name="postpone_reason" id="postpone-reason" class="form-control"
                            value="{{ old('postpone_reason', $ot->postpone_reason) }}" style="display:none"
                            placeholder="Reason for postponement">
                        <input type="text" name="cancellation_reason" id="cancel-reason" class="form-control"
                            value="{{ old('cancellation_reason', $ot->cancellation_reason) }}" style="display:none"
                            placeholder="Reason for cancellation">
                        <input type="date" name="rescheduled_date" id="reschedule-date" class="form-control mt-2"
                            value="{{ old('rescheduled_date', $ot->rescheduled_date?->format('Y-m-d')) }}"
                            style="display:none">
                    </div>
                </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="surgeon_id" class="form-select" required>
                            @foreach ($surgeons as $doc)
                                <option value="{{ $doc->id }}"
                                    {{ old('surgeon_id', $ot->surgeon_id) == $doc->id ? 'selected' : '' }}>
                                    Dr. {{ $doc->employee?->first_name }} {{ $doc->employee?->last_name }} —
                                    {{ $doc->specialization }}
                                </option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <select name="anesthesiologist_id" class="form-select">
                            ...
                            @foreach ($anesthesiologists as $doc)
                                <option value="{{ $doc->id }}"
                                    {{ old('anesthesiologist_id', $ot->anesthesiologist_id) == $doc->id ? 'selected' : '' }}>
                                    Dr. {{ $doc->employee?->first_name }} {{ $doc->employee?->last_name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-section">
                    ...
                    <div class="mb-3">
                        <label class="check-card" for="consent_obtained">
                            ...
                            <div>
                                ...
                                <div class="check-card-sub">Patient / guardian signed</div>
                            </div>
                        </label>
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="text" name="consent_by" class="form-control"
                            value="{{ old('consent_by', $ot->consent_by) }}">
                    </div>
                    <label class="check-card" for="pre_op_assessment_done">
                        ...
                        <div>
                            ...
                            <div class="check-card-sub">Fitness confirmed</div>
                        </div>
                    </label>
                </div>
                ...
                <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px">
                    <button type="submit" class="btn btn-warning w-100 mb-2"
                        style="height:40px;font-size:14px;font-weight:500">
                        <i class="bi bi-check-lg me-2"></i>Save Changes
                    </button>
                    <a href="{{ route('ot.schedules.show', $ot->id) }}" class="btn btn-outline-secondary w-100"
                        style="height:40px;font-size:14px">Cancel</a>
                </div>
