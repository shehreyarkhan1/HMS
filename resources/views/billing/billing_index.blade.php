...
<style>
    ...
    .badge-Draft      { background:#f1f5f9;color:#475569;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500 }
    .badge-Finalized  { background:#dcfce7;color:#166534;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500 }
    ...
    .pay-Partial { background:#dbeafe;color:#1e40af;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500 }
    .pay-Paid    { background:#dcfce7;color:#166534;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500 }
    .type-OPD       { background:#f0fdf4;color:#15803d;font-size:10px;padding:2px 8px;border-radius:20px;font-weight:500 }
    .type-IPD       { background:#eff6ff;color:#1d4ed8;font-size:10px;padding:2px 8px;border-radius:20px;font-weight:500 }
    .type-Emergency { background:#fef2f2;color:#dc2626;font-size:10px;padding:2px 8px;border-radius:20px;font-weight:700 }
</style>
@endpush
...
<div class="card-section">
    <div class="card-section-header">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            <form method="GET" action="{{ route('billing.index') }}" class="d-flex gap-2 flex-wrap filter-bar">
                <input type="text" name="search" placeholder="Bill #, patient, MRN..." value="{{ request('search') }}" style="width:200px">
                <select name="status">
                ...
                @if(request()->hasAny(['search','status','payment_status','bill_type','date_from','date_to']))
                    <a href="{{ route('billing.index') }}" class="btn btn-sm btn-outline-secondary px-3" style="height:36px;font-size:13px">Clear</a>
                @endif
            </form>
            <a href="{{ route('billing.service-charges.index') }}" class="btn btn-sm btn-outline-secondary px-3" style="height:36px;font-size:13px">
                <i class="bi bi-gear me-1"></i>Service Charges
            </a>
            <a href="{{ route('billing.create') }}" class="btn btn-sm btn-success px-3" style="height:36px;font-size:13px">
        ...
        <table class="table table-clean mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:13px;font-weight:600;color:#1e293b">Rs. {{ number_format($bill->net_amount, 0) }}</td>
                    <td style="font-size:12px;font-weight:500;color:#16a34a">Rs. {{ number_format($bill->paid_amount, 0) }}</td>
                    <td style="font-size:12px;font-weight:500;color:{{ $bill->due_amount > 0 ? '#dc2626' : '#94a3b8' }}">
                    ...
                    <td>
                        <div class="d-flex justify-content-end gap-1">
                            <a href="{{ route('billing.show', $bill) }}" class="btn btn-sm btn-outline-primary py-0 px-2" style="font-size:12px" title="View">
                                <i class="bi bi-eye"></i>
                            </a>
                            @if($bill->isDraft())
                            <a href="{{ route('billing.edit', $bill) }}" class="btn btn-sm btn-outline-warning py-0 px-2" style="font-size:12px" title="Edit">
                                <i class="bi bi-pencil"></i>
                            </a>
                            @endif
                            @if($bill->isFinalized() && !$bill->isPaid())
                            <a href="{{ route('billing.show', $bill) }}#payment" class="btn btn-sm btn-success py-0 px-2" style="font-size:12px" title="Record Payment">
                                <i class="bi bi-cash-coin"></i>
                            </a>
                            @endif
                            <a href="{{ route('billing.print', $bill) }}" target="_blank" class="btn btn-sm btn-outline-secondary py-0 px-2" style="font-size:12px" title="Print Invoice">
    ...
    <div class="d-flex justify-content-between align-items-center px-4 py-3" style="border-top:1px solid #e2e8f0">
        <span style="font-size:12px;color:#94a3b8">Showing {{ $bills->firstItem() }}–{{ $bills->lastItem() }} of {{ $bills->total() }}</span>
