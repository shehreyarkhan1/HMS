...
<style>
    ...
    .table-clean td {
        font-size:13px;color:#374151;padding:11px 16px !important;
    ...
    .region-badge   { background:#ede9fe;color:#5b21b6;font-size:11px;padding:3px 10px;border-radius:20px;font-weight:500 }
    ...
    .modal-header h6 { margin:0;font-size:15px;font-weight:600;color:#1e293b }
    .modal-close {
    ...
    .toggle-row label { font-size:13px;color:#374151;font-weight:500;margin:0;cursor:pointer }
</style>
@endpush
                            ...
                            <div class="d-flex justify-content-end gap-1">
                                ...
                                <button type="button"
                                    onclick="openEditModal(
                                        ...
                                        '{{ addslashes($bp->name) }}',
                                        '{{ addslashes($bp->code) }}',
                                        '{{ addslashes($bp->region ?? '') }}',
                                        {{ $bp->is_active ? 'true' : 'false' }}
                                    )"
                                ...
                                {{-- Toggle --}}
                                <form method="POST"
                                    action="{{ route('radiology.body-parts.toggle', $bp->id) }}"
                                    class="d-inline">
                                    @csrf
                                ...
                                {{-- Delete --}}
                                <form method="POST"
                                    action="{{ route('radiology.body-parts.destroy', $bp->id) }}"
                                    class="d-inline"
                                    onsubmit="return confirm('Delete {{ addslashes($bp->name) }}?')">
                                    @csrf @method('DELETE')
...
<script>
    ...
    function openEditModal(id, name, code, region, isActive) {
        ...
        for (let o of sel.options) o.selected = (o.value === region);

        openModal('editModal');
        setTimeout(() => document.getElementById('edit_name').focus(), 180);
    }

    {{-- Agar validation fail ho aur add form tha to modal dobara kholo --}}
