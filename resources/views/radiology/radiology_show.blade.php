...
@section('content')
    ...
    @if (session('success'))
        <div class="alert alert-success d-flex align-items-center gap-2 mb-4" style="border-radius:10px;font-size:13px">
    ...
    <div class="row g-3 mb-4">
        ...
        {{-- Right col: Billing (radiologist/admin) ya Status (doctor) --}}
        @if ($isDoctor)
            <div class="col-md-4">
                <div class="info-card h-100">
                    <div class="info-card-title"><i class="bi bi-info-circle text-primary"></i> Order Status</div>
                    <div class="info-row">
                        <span class="info-label">Status</span>
                        <span class="info-value">{{ $radiologyOrder->status }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Priority</span>
                        <span class="info-value">{{ $radiologyOrder->priority }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Scheduled</span>
                        <span class="info-value">
                            {{ $radiologyOrder->scheduled_at ? $radiologyOrder->scheduled_at->format('d M Y H:i') : 'Not scheduled' }}
                        </span>
                    </div>
                    @if ($radiologyOrder->report_delivered)
                        <div class="info-row">
                    ...
                    @endif
                    @if ($radiologyOrder->hasCriticalFindings())
                        <div class="mt-3 p-2" style="background:#fee2e2;border:1px solid #fca5a5;border-radius:6px;">
                            <span style="font-size:12px;color:#991b1b;font-weight:600">
                                <i class="bi bi-exclamation-circle-fill me-1"></i>
                                Critical findings present — review report carefully.
                            </span>
                        </div>
                    @endif
                </div>
                ...
                <div class="info-card h-100">
                    ...
                    @if ($radiologyOrder->discount > 0)
                        <div class="info-row">
                            ...
                            <span class="info-value" style="color:#dc2626">-Rs
                                {{ number_format($radiologyOrder->discount, 0) }}</span>
                        </div>
                    @endif
                    <div class="info-row">
                        <span class="info-label">Net Amount</span>
                        <span class="info-value" style="font-size:15px">Rs
                            {{ number_format($radiologyOrder->net_amount, 0) }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Paid</span>
                        <span class="info-value" style="color:#16a34a">Rs
                            {{ number_format($radiologyOrder->paid_amount, 0) }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Balance</span>
                        <span class="info-value"
                            style="color:{{ $radiologyOrder->balance_due > 0 ? '#dc2626' : '#16a34a' }}">
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value">
                            <span
                                class="pay-{{ $radiologyOrder->payment_status }}">{{ $radiologyOrder->payment_status }}</span>
                        </span>
                    </div>
        ...
        <div class="section-body">
            @foreach ($radiologyOrder->items as $item)
                ...
                <div class="exam-item {{ $itemCls }}">
                        ...
                        <div class="d-flex gap-2 flex-wrap mt-2">
                            @foreach ($item->images as $img)
                                @if (in_array($img->file_type, ['image']))
                                    <img src="{{ Storage::url($img->file_path) }}" class="img-thumb"
                    ...
                    @if ($item->report)
                        <div class="report-box" id="report-{{ $item->id }}">
                            @if ($item->report->is_critical)
                                <div
                                    style="background:#fee2e2;border-radius:6px;padding:8px 12px;margin-bottom:10px;font-size:12px;color:#991b1b;font-weight:600">
                    ...
                    {{-- COMPLETE SCAN FORM — doctor nahi dekhe --}}
                    @if ($item->status === 'In Progress' && !$isDoctor)
                            ...
                            <form method="POST"
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-md-9 contrast-fields-{{ $item->id }}" style="display:none">
                                        <div class="row g-2">
                                            <div class="col-md-4">
                                                <input type="text" name="contrast_agent"
                                                    placeholder="Agent (e.g. Omnipaque)" class="form-control-sm2">
                                            </div>
                                            <div class="col-md-3">
                                            ...
                                            <div class="col-md-5">
                                                <label class="form-label-sm">
                                                    <input type="checkbox" name="contrast_reaction" value="1"
                                                        class="me-1">
                                                    Reaction occurred
                                                </label>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="file" name="images[]" multiple accept=".jpg,.jpeg,.png,.pdf,.dcm"
                    ...
                    {{-- REPORT ENTRY FORM — doctor nahi dekhe --}}
                    @if (in_array($item->status, ['Scan Completed']) && !$item->report && !$isDoctor)
                        <div class="form-section" id="report">
                            <div style="font-size:12px;font-weight:600;color:#374151;margin-bottom:12px">
                                <i class="bi bi-file-earmark-medical me-1 text-success"></i> Enter Radiology Report
                            </div>
                            <form method="POST"
                                action="{{ route('radiology.orders.storeReport', $radiologyOrder->id) }}">
                                @csrf
                                <input type="hidden" name="item_id" value="{{ $item->id }}">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea name="findings" class="form-control-sm2" rows="5" placeholder="Detailed radiological findings..."
                                            required></textarea>
                                    </div>
                                    <div class="col-12">
                                        <label class="form-label-sm">Impression / Conclusion *</label>
                                        <textarea name="impression" class="form-control-sm2" rows="3" placeholder="Final diagnosis / impression..."
                                            required></textarea>
                                    </div>
                                    <div class="col-12">
                                    ...
                                    <div class="col-12">
                                        <div class="d-flex align-items-center gap-3 flex-wrap">
                                            <label style="font-size:12px;cursor:pointer">
                                                ...
                                                <span style="color:#dc2626;font-weight:600">⚠ Mark as Critical
                                                    Finding</span>
                                            </label>
                                        </div>
                                        <div id="critical-fields" style="display:none;margin-top:10px">
                                            <div class="row g-2">
                                                <div class="col-md-6">
                                                    ...
                                                    <textarea name="critical_notes" class="form-control-sm2" rows="2" placeholder="Describe critical finding..."></textarea>
                                                </div>
                                                <div class="col-md-6">
                                                    <label class="form-label-sm">Notify Doctor *</label>
                                                    <input type="text" name="critical_notified_to"
                                                        class="form-control-sm2" placeholder="Doctor name notified">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
            ...
            {{-- Schedule form — doctor nahi dekhe --}}
            @if ($radiologyOrder->status === 'Pending' && !$isDoctor)
                <div class="form-section mt-2" id="schedule">
                    ...
                    <form method="POST" action="{{ route('radiology.orders.schedule', $radiologyOrder->id) }}"
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="scheduled_at" class="form-control-sm2"
    ...
    {{-- Cancel — doctor nahi dekhe --}}
    @if ($radiologyOrder->isCancellable() && !$isDoctor)
