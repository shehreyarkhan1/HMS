...
@push('styles')
    <style>
        ...
        .bed-Available .bed-number {
            color: #15803d
        }

        .bed-Occupied .bed-number {
            color: #dc2626
        }

        .bed-Reserved .bed-number {
            color: #ca8a04
        }

        .bed-Maintenance .bed-number {
            color: #6b7280
        }
        ...
        .bed-Available .bed-status-label {
            color: #16a34a
        }

        .bed-Occupied .bed-status-label {
            color: #dc2626
        }

        .bed-Reserved .bed-status-label {
            color: #ca8a04
        }

        .bed-Maintenance .bed-status-label {
            color: #6b7280
        }
...
@section('content')
    ...
    <div class="row g-3">
        ...
        <div class="col-12">
            <div
                style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
                <div class="d-flex align-items-center gap-3">
                    <div
                        style="width:44px;height:44px;border-radius:10px;background:#eff6ff;display:flex;align-items:center;justify-content:center">
                    ...
                    <div>
                        ...
                        <div style="font-size:12px;color:#94a3b8">
                            ...
                            @if ($ward->floor)
                                &bull; {{ $ward->floor }} Floor
                            @endif
                            @if ($ward->block)
                                &bull; Block {{ $ward->block }}
                            @endif
                        </div>
                    </div>
                </div>
                <div class="d-flex gap-3" style="font-size:13px">
                    <span style="color:#16a34a"><i class="bi bi-circle-fill me-1"
                            style="font-size:8px"></i>{{ $ward->beds->where('status', 'Available')->count() }}
                        Available</span>
                    <span style="color:#dc2626"><i class="bi bi-circle-fill me-1"
                            style="font-size:8px"></i>{{ $ward->beds->where('status', 'Occupied')->count() }}
                        Occupied</span>
                    <span style="color:#ca8a04"><i class="bi bi-circle-fill me-1"
                            style="font-size:8px"></i>{{ $ward->beds->where('status', 'Reserved')->count() }}
                        Reserved</span>
                    <span style="color:#9ca3af"><i class="bi bi-circle-fill me-1"
                            style="font-size:8px"></i>{{ $ward->beds->where('status', 'Maintenance')->count() }}
                        Maint.</span>
                </div>
                <div class="d-flex gap-2">
                    <span
                        style="font-size:13px;font-weight:600;color:#15803d;background:#f0fdf4;padding:4px 12px;border-radius:8px">
                        Rs {{ number_format($ward->bed_charges, 0) }}/day
                    </span>
                    <a href="{{ route('wards.edit', $ward->id) }}" class="btn btn-sm btn-outline-warning"
                        style="font-size:13px">
                        <i class="bi bi-pencil me-1"></i>Edit
                    </a>
                    <a href="{{ route('wards.index') }}" class="btn btn-sm btn-outline-secondary" style="font-size:13px">
                        Back
                    </a>
                </div>
            </div>
                ...
                <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:16px 20px">
                    ...
                    <form method="POST" action="{{ route('wards.assign-bed') }}" class="d-flex gap-2 flex-wrap">
                        ...
                        <select name="patient_id" class="form-select"
                            style="max-width:240px;font-size:13px;height:36px;border:1px solid #bfdbfe;border-radius:8px">
                        ...
                        <select name="bed_id" class="form-select"
                            style="max-width:200px;font-size:13px;height:36px;border:1px solid #bfdbfe;border-radius:8px">
            ...
            <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px">
                ...
                <div class="bed-grid">
                    @foreach ($ward->beds->sortBy('bed_number') as $bed)
                        <div class="bed-card bed-{{ $bed->status }}" data-bed-id="{{ $bed->id }}"
                            data-status="{{ $bed->status }}"
                            onclick="showBedActions({{ $bed->id }}, '{{ $bed->status }}', '{{ $bed->bed_number }}')">

                            <i
                                class="bi {{ $bed->status == 'Available' ? 'bi-check-circle' : ($bed->status == 'Occupied' ? 'bi-person-fill' : ($bed->status == 'Reserved' ? 'bi-clock' : 'bi-tools')) }} bed-icon"></i>
                            ...
                            @if ($bed->status == 'Occupied' && $bed->patient)
                                ...
                                <a href="{{ route('ward.patient.show', $bed->patient_id) }}"
                                    onclick="event.stopPropagation()" class="btn btn-primary w-100 mt-2"
                                    style="font-size:10px;padding:3px 0">
                                    <i class="bi bi-person-lines-fill me-1"></i>View Chart
                                </a>
                            @endif
                        </div>
                    @endforeach
    ...
    {{-- Action Modal --}}
    <div id="bedModal"
        style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:999;align-items:center;justify-content:center">
        <div style="background:#fff;border-radius:12px;padding:24px;width:340px;max-width:90vw">
            ...
            <button onclick="closeModal()" class="btn btn-outline-secondary w-100 mt-2"
