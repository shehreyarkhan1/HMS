...
@section('content')
    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        {{-- <a href="{{ route('wards.create') }}" class="btn btn-sm btn-success px-3"
            style="height:36px;font-size:13px;line-height:2.2">
                    ...
                    <div class="ward-card">
                        <div class="ward-card-header">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @if (!$ward->is_active)
                                    <span
                                        style="font-size:10px;padding:2px 7px;border-radius:10px;background:#f3f4f6;color:#6b7280">Inactive</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="ward-card-body">
                            ...
                            <div class="bed-bar-wrap">
                                <div class="bed-bar-fill"
                                    style="width:{{ $pct }}%;background:{{ $barColor }}"></div>
                            </div>

                            {{-- Bed Status --}}
                            <div class="d-flex gap-3 mt-2 mb-3" style="font-size:12px">
                                <span style="color:#16a34a"><i class="bi bi-circle-fill" style="font-size:7px"></i>
                                    {{ $available }} Available</span>
                                <span style="color:#dc2626"><i class="bi bi-circle-fill" style="font-size:7px"></i>
                                    {{ $occupied }} Occupied</span>
                                @if ($reserved)
                                    <span style="color:#d97706"><i class="bi bi-circle-fill" style="font-size:7px"></i>
                                        {{ $reserved }} Reserved</span>
                                @endif
                                @if ($maint)
                                    <span style="color:#6b7280"><i class="bi bi-circle-fill" style="font-size:7px"></i>
                                        {{ $maint }} Maint.</span>
                                @endif
                            </div>
                            ...
                            {{-- Info --}}
                            <div class="info-grid">
                                <div class="info-item"><i class="bi bi-grid-3x2"></i>{{ $ward->total_beds }} total beds
                                </div>
                                <div class="info-item"><i class="bi bi-cash"></i>Rs
                                    {{ number_format($ward->bed_charges, 0) }}/day</div>
                                @if ($ward->floor)
                            ...
                            <div class="d-flex gap-2 mt-3">
                                <a href="{{ route('wards.show', $ward->id) }}" class="btn btn-sm btn-primary flex-grow-1"
