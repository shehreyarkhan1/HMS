...
<style>
    /* -- Stat Cards -- */
    .stat-card{background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:1.1rem 1.25rem;height:100%;transition:box-shadow .15s}
    .stat-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.07)}
    .stat-label{font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.4rem}
    .stat-value{font-size:28px;font-weight:700;color:#1e293b;line-height:1}
    .stat-sub{font-size:11px;color:#94a3b8;margin-top:.3rem}
    ...
    /* -- Section Cards -- */
    .section-card{background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;height:100%}
    .section-header{padding:12px 16px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between}
    .section-title{font-size:13px;font-weight:600;color:#1e293b;margin:0}

    /* -- Patient Row -- */
    .patient-row{display:flex;align-items:center;padding:10px 16px;border-bottom:1px solid #f8fafc;gap:10px;transition:background .1s}
    .patient-row:last-child{border-bottom:none}
    .patient-row:hover{background:#f8fafc}
    .patient-avatar{width:34px;height:34px;border-radius:50%;background:#dbeafe;color:#1d4ed8;font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0}
    .patient-name{font-size:13px;font-weight:500;color:#1e293b}
    .patient-sub{font-size:11px;color:#94a3b8;margin-top:1px}

    /* -- Order Row -- */
    .order-row{padding:10px 16px;border-bottom:1px solid #f8fafc;display:flex;align-items:flex-start;gap:10px}
    .order-row:last-child{border-bottom:none}
    .order-priority{width:6px;border-radius:3px;align-self:stretch;flex-shrink:0}
    .priority-STAT{background:#dc2626}
    .priority-Urgent{background:#d97706}
    .priority-Routine{background:#16a34a}

    /* -- Vitals Due -- */
    .vital-due-row{padding:10px 16px;border-bottom:1px solid #f8fafc;display:flex;align-items:center;gap:10px}
    .vital-due-row:last-child{border-bottom:none}
    .pulse-dot{width:8px;height:8px;border-radius:50%;background:#dc2626;animation:pulse 1.2s infinite;flex-shrink:0}
    @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}}

    /* -- My Activity -- */
    .activity-row{padding:9px 16px;border-bottom:1px solid #f8fafc;display:flex;align-items:center;gap:10px}
    .activity-row:last-child{border-bottom:none}
    .activity-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0}

    /* -- Shift Badge -- */
    .shift-badge{padding:4px 14px;border-radius:20px;font-size:12px;font-weight:600}
    .shift-Morning{background:#fefce8;color:#ca8a04;border:1px solid #fde68a}
    .shift-Evening{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}
    .shift-Night{background:#1e1b4b;color:#a5b4fc;border:1px solid #3730a3}

    .empty-msg{padding:2rem;text-align:center;color:#94a3b8;font-size:13px}
    .empty-msg i{font-size:26px;display:block;margin-bottom:.5rem}
...
{{-- Welcome Banner --}}
<div style="background:linear-gradient(135deg,#1d4ed8 0%,#0369a1 100%);border-radius:14px;padding:20px 24px;margin-bottom:20px;color:#fff">
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            @if($stats['urgent_orders'] > 0)
                <span style="background:#dc2626;color:#fff;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:600;animation:pulse 1.5s infinite">
...
<div class="row g-3">
        ...
        <div class="section-card mb-3" style="border-color:#fecaca">
            ...
            <div class="vital-due-row">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="patient-sub">Bed {{ $bed->bed_number }} — {{ $bed->ward->name ?? '—' }}</div>
                </div>
        ...
        <div class="section-card">
            ...
            <div class="patient-row">
                <div class="patient-avatar">
                    {{ strtoupper(substr($bed->patient->name ?? 'P', 0, 1)) }}
                </div>
                <div class="flex-grow-1">
                    <div class="patient-name">{{ $bed->patient->name ?? '—' }}</div>
                    <div class="patient-sub">
                        Bed {{ $bed->bed_number }}
                        @if($bed->ward) &bull; {{ $bed->ward->name }} @endif
                        @if($bed->admitted_at) &bull; {{ \Carbon\Carbon::parse($bed->admitted_at)->diffInDays(now()) }}d @endif
                    </div>
                </div>
                <a href="{{ route('ward.patient.show', $bed->patient_id) }}"
            ...
            @empty
            <div class="empty-msg">
                <i class="bi bi-people"></i>No patients admitted
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div class="col-12 col-lg-5">
        ...
        <div class="section-card mb-3">
            <div class="section-header">
                ...
                <span class="badge bg-warning text-dark" style="font-size:10px">{{ $pendingOrders->count() }}</span>
                @endif
            </div>
            ...
            <div class="order-row">
                ...
                <div class="flex-grow-1">
                    ...
                    <div style="font-size:11px;color:#94a3b8;margin-top:3px">
                        ...
                        Dr. {{ $order->doctor->employee->full_name ?? ($order->doctor->employee->first_name ?? 'Unknown Doctor') }}
                        &bull;
            ...
            @empty
            <div class="empty-msg">
                <i class="bi bi-clipboard2-check"></i>No pending orders
            </div>
            @endforelse
        </div>
        ...
        <div class="section-card" style="border-color:#fde68a">
            <div class="section-header" style="background:#fffbeb">
                <span class="section-title" style="color:#b45309">
                    ...
                    <span class="badge bg-warning text-dark ms-1" style="font-size:10px">{{ $urgentNotes->count() }}</span>
                </span>
            </div>
            ...
            <div class="patient-row">
                ...
                <div class="flex-grow-1">
                    ...
                    <div style="font-size:11px;color:#94a3b8">
                        <i class="bi bi-person-nurse me-1"></i>By: {{ $note->nurse->name ?? '—' }}
    ...
    <div class="col-12 col-lg-3">
        ...
        <div class="section-card mb-3">
            ...
            <div style="padding:8px 16px;font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #f1f5f9">
            ...
            <div class="activity-row">
                ...
                <div class="flex-grow-1">
                    <div style="font-size:12px;font-weight:500;color:#1e293b">{{ $vital->patient->name ?? '—' }}</div>
                    <div style="font-size:11px;color:#94a3b8">
                        {{ $vital->shift }} shift &bull; {{ $vital->recorded_at->format('h:i A') }}
                        @if($vital->systolic_bp) &bull; BP {{ $vital->systolic_bp }}/{{ $vital->diastolic_bp }} @endif
                    </div>
                </div>
            </div>
            @endforeach
            ...
            <div style="padding:8px 16px;font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #f1f5f9;border-top:1px solid #f1f5f9">
            ...
            <div class="activity-row">
                ...
                <div class="flex-grow-1">
                    <div style="font-size:12px;font-weight:500;color:#1e293b">{{ $note->patient->name ?? '—' }}</div>
                    <div style="font-size:11px;color:#94a3b8">
            ...
            <div class="empty-msg">
                <i class="bi bi-clipboard2"></i>No activity yet today
            </div>
            @endif
        </div>
        ...
        <div class="section-card">
            ...
            <div style="padding:12px 16px;display:grid;gap:8px">
                <a href="{{ route('wards.index') }}"
                   class="btn btn-outline-primary btn-sm text-start" style="font-size:13px">
                    <i class="bi bi-building me-2"></i>View All Wards
                </a>
                <a href="{{ route('appointments.index') }}"
                   class="btn btn-outline-secondary btn-sm text-start" style="font-size:13px">
