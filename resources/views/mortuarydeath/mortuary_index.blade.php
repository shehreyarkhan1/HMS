                ...
                <form method="GET" action="{{ route('mortuary.index') }}" class="filter-bar">
                    ...
                    <select name="manner">
                        ...
                        @foreach (['Natural', 'Accidental', 'Homicidal', 'Suicidal', 'Undetermined'] as $m)
                            <option value="{{ $m }}" {{ request('manner') == $m ? 'selected' : '' }}>
                                {{ $m }}</option>
                        @endforeach
                    </select>
                    ...
                    @if (request()->hasAny(['search', 'status', 'manner', 'mlc', 'date_from', 'date_to']))
                        <a href="{{ route('mortuary.index') }}" class="btn btn-sm btn-outline-secondary px-3"
                            style="height:36px;font-size:13px">Clear</a>
                    @endif
                </form>

                <a href="{{ route('mortuary.create') }}" class="btn btn-sm btn-danger px-3"
                    ...
                    @forelse($records as $record)
                        <tr>
                            ...
                            <td>
                                ...
                                <div style="font-size:11px;color:#94a3b8">{{ $record->patient?->mrn }} ·
                                    {{ $record->patient?->age }} yrs / {{ $record->patient?->gender }}</div>
                            </td>
                            <td>
                                <div style="font-weight:500">
                                    {{ \Carbon\Carbon::parse($record->death_datetime)->format('d M Y') }}</div>
                                <div style="font-size:11px;color:#94a3b8">
                                    {{ \Carbon\Carbon::parse($record->death_datetime)->format('h:i A') }}</div>
                            </td>
                            <td>
                                <span style="font-size:12px">{{ $record->death_location }}</span>
                                @if ($record->ward)
                                    <div style="font-size:11px;color:#94a3b8">{{ $record->ward }}
                                        {{ $record->bed_number ? '/ Bed ' . $record->bed_number : '' }}</div>
                                @endif
                            </td>
                            <td style="max-width:160px">
                                <div style="font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"
                                    title="{{ $record->immediate_cause }}">
