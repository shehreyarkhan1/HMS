...
<style>
    ...
    .form-control-clean:focus, .form-select-clean:focus {
        border-color: #93c5fd; background: #fff; box-shadow: 0 0 0 3px rgba(59,130,246,.08);
    }
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-8">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                @if($patient)
                                    ...
                                    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 14px;font-size:13px;color:#1e40af">
                                        ...
                                        <span style="color:#94a3b8;font-size:11px;margin-left:6px">{{ $patient->mrn }}</span>
                                    </div>
                                @else
                                    <select name="patient_id" class="form-select-clean" required>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="death_location" class="form-select-clean" required>
                                    @foreach(['Ward','ICU','CCU','Emergency','OT','DOA','Outside Hospital'] as $loc)
                                        <option value="{{ $loc }}" {{ old('death_location') == $loc ? 'selected' : '' }}>{{ $loc }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="ward" class="form-control-clean"
                                       value="{{ old('ward') }}" placeholder="e.g. Medical A">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="manner_of_death" class="form-select-clean" required>
                                    @foreach(['Natural','Accidental','Homicidal','Suicidal','Undetermined'] as $m)
                                        <option value="{{ $m }}" {{ old('manner_of_death', 'Natural') == $m ? 'selected' : '' }}>{{ $m }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                                ...
                                <select name="declared_by" class="form-select-clean">
                                    ...
                                    @foreach($doctors as $doc)
                                        <option value="{{ $doc->id }}" {{ old('declared_by') == $doc->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="body_condition" class="form-select-clean">
                                    ...
                                    @foreach(['Normal','Decomposed','Burned','Traumatic','Other'] as $c)
                                        <option value="{{ $c }}" {{ old('body_condition') == $c ? 'selected' : '' }}>{{ $c }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-6" id="postmortemOrderedBy" style="display:none">
                                ...
                                <select name="postmortem_ordered_by" class="form-select-clean">
                                    ...
                                    @foreach(['Doctor','Police','Court','Hospital'] as $o)
                                        <option value="{{ $o }}" {{ old('postmortem_ordered_by') == $o ? 'selected' : '' }}>{{ $o }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-12">
                                <label style="font-size:13px;cursor:pointer;display:flex;align-items:center;gap:8px">
                                    <input type="checkbox" name="is_medico_legal" value="1"
                                           id="mlcCheckbox"
                                           {{ old('is_medico_legal') ? 'checked' : '' }}
                                           onchange="toggleMlc(this.checked)"
                ...
                <div class="card-section">
                    <div class="section-body">
                        ...
                        <textarea name="notes" class="form-control-clean" rows="3"
            ...
            <div class="col-lg-4">
                <div style="position:sticky;top:80px;display:flex;flex-direction:column;gap:16px">
                    ...
                    <div class="card-section">
                        <div class="card-section-header">
                            <div class="header-icon" style="background:#f0fdf4;color:#15803d"><i class="bi bi-person-badge"></i></div>
                        ...
                        <div class="section-body">
                            ...
                            <select name="admitted_by" class="form-select-clean">
                                ...
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" {{ old('admitted_by') == $emp->id ? 'selected' : '' }}>
...
<script>
    function toggleMlc(show) {
        ...
        if (show) {
            ...
            if (pmYes) { pmYes.checked = true; showPostmortem(true); }
        }
    }
