        ...
        <div class="row g-4">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="date_of_birth" class="form-control"
                                value="{{ old('date_of_birth', $donor->date_of_birth->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="gender" class="form-select" required>
                                @foreach (['Male', 'Female', 'Other'] as $g)
                                    <option value="{{ $g }}"
                                        {{ old('gender', $donor->gender) == $g ? 'selected' : '' }}>{{ $g }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <input type="number" name="weight_kg" class="form-control"
                                value="{{ old('weight_kg', $donor->weight_kg) }}" step="0.1" min="45">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">CNIC</label>
                            <input type="text" name="cnic" class="form-control"
                                value="{{ old('cnic', $donor->cnic) }}">
                            @error('cnic')
                                <div class="text-danger" style="font-size:11px;margin-top:3px">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="col-12 col-xl-4">
                    ...
                    <div>
                        ...
                        <select name="donor_type" class="form-select" required>
                            @foreach (['Voluntary', 'Replacement', 'Autologous', 'Directed'] as $t)
                                <option value="{{ $t }}"
                                    {{ old('donor_type', $donor->donor_type) == $t ? 'selected' : '' }}>
                                    {{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    {{-- <select name="patient_id" class="form-select">
                        ...
                        @foreach ($patients as $patient)
                            <option value="{{ $patient->id }}"
                                {{ old('patient_id', $donor->patient_id) == $patient->id ? 'selected' : '' }}>
                ...
                <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:20px">
                    ...
                    <a href="{{ route('blood-bank.donors.show', $donor->id) }}" class="btn btn-outline-secondary w-100"
