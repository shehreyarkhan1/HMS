...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route('blood-bank.index') }}" class="btn btn-sm btn-outline-secondary px-3"
    ...
    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:14px 20px;margin-bottom:16px">
        <form method="GET" action="{{ route('blood-bank.requests.index') }}"
            ...
            <input type="text" name="search" placeholder="Request ID, patient name, MRN..."
                    ...
                    <div class="req-card {{ $urgClass }}">
                        ...
                        <div class="d-flex gap-1 flex-wrap mb-2">
                            ...
                            @if ($req->units_approved > 0)
                                <span
                                    style="font-size:10px;background:#f0fdf4;color:#15803d;padding:2px 8px;border-radius:6px;font-weight:600">
                        ...
                        <div class="d-flex gap-1 align-items-center">
                            ...
                            @if (in_array($req->status, ['Pending', 'Under Review']))
                                <button type="button" class="btn btn-sm btn-outline-success py-0 px-2 approve-btn"
                                    style="font-size:12px" data-bs-toggle="modal" data-bs-target="#approveModal"
                                    data-id="{{ $req->id }}" data-units="{{ $req->units_required }}">
                                    <i class="bi bi-check2 me-1"></i>Approve
                                </button>
                            @endif

                            @if (!in_array($req->status, ['Fulfilled', 'Crossmatch']))
                                <form method="POST" action="{{ route('blood-bank.requests.destroy', $req->id) }}"
                                    onsubmit="return confirm('Remove request {{ $req->request_id }}?')" class="ms-auto">
                                    @csrf @method('DELETE')
                    ...
                    <div class="modal-body" style="padding:24px">
                        <div class="row g-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" name="units_required" class="form-control" value="1"
                                    min="1" max="20" required>
                            </div>

                            <div class="col-md-2">
                                <label class="form-label">Hb (g/dL)</label>
                                <input type="number" name="patient_hemoglobin" class="form-control" step="0.1"
                                    min="1" max="25" placeholder="e.g. 7.2">
                            </div>
                    ...
                    <div class="modal-body" style="padding:24px">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="number" name="units_approved" id="approve-units" class="form-control"
                                    min="1" max="20" required placeholder="How many units to approve">
                                <div style="font-size:11px;color:#94a3b8;margin-top:4px">
